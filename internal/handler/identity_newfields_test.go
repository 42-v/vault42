...
// WS1.4: the new BeOn3 profile fields must round-trip through PUT → GET.
func TestIdentityPut_NewFieldsRoundTrip(t *testing.T) {
	...
	body := `{"given_name":"Ada","username":"ada","country":"GB","state":"ENG","sex":"female","marketing_emails":true,"dynamic":{"beon3.forum":{"reputation":42}}}`
	...
	if string(resp.Dynamic["beon3.forum"]) != `{"reputation":42}` {
		t.Fatalf("dynamic lost: %s", resp.Dynamic["beon3.forum"])
	}
}
...
func TestIdentityPut_DynamicAbuseRejected(t *testing.T) {
	...
	body := `{"dynamic":{"BeOn3 Bad Key":{"x":1}}}`
	req := setAuthContext(httptest.NewRequest(http.MethodPut, "/user/identity", strings.NewReader(body)), "user-123")
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	h.Put(rec, req)
