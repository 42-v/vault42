...
func TestIdentityData_Validate(t *testing.T) {
	...
	big["beon3.blob"] = json.RawMessage(`"` + strings.Repeat("x", dynamicMaxBytes) + `"`)
	...
	tests := []struct {
		...
		{"full valid profile", IdentityData{
			...
			Dynamic:         map[string]json.RawMessage{"beon3.forum": json.RawMessage(`{"reputation":42}`)},
		}, false},
		...
		{"dynamic key uppercase", IdentityData{Dynamic: map[string]json.RawMessage{"BeOn3": json.RawMessage(`1`)}}, true},
		{"dynamic value invalid json", IdentityData{Dynamic: map[string]json.RawMessage{"beon3.x": json.RawMessage(`{`)}}, true},
...
func TestIdentityData_RoundTrip(t *testing.T) {
	in := IdentityData{
		...
		Dynamic: map[string]json.RawMessage{"beon3.garage": json.RawMessage(`{"vehicles":3}`)},
	}
	...
	if string(out.Dynamic["beon3.garage"]) != `{"vehicles":3}` {
		t.Fatalf("dynamic lost: %s", out.Dynamic["beon3.garage"])
	}
}
