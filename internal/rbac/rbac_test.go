...
func TestPermissionsForRole_Counts(t *testing.T) {
	...
	if len(operatorPerms) != 17 {
		t.Errorf("operator should have 17 permissions, got %d", len(operatorPerms))
	}
	if len(superAdminPerms) != 32 {
		t.Errorf("super_admin should have 32 permissions, got %d", len(superAdminPerms))
	}
