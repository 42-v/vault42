...
### AR-9: Admin Client Certificate Identity Pinning Is Optional

**Severity:** Low (by design) | **Source:** Admin gateway security review M3, revised for 1.0.0

The TLS stack requires a client certificate signed by `ADMIN_GW_CLIENT_CA_FILE`. Identity pinning and revocation are implemented (`cmd/admin-gateway/clientauth.go`) and are optional:

- `ADMIN_GW_CLIENT_CN_ALLOWLIST` pins the leaf's subject CN and its DNS, email and URI SANs by exact match. Empty pins nothing.
- `ADMIN_GW_CLIENT_CRL_FILE` is a PEM or DER revocation list signed by the client CA, re-read on every handshake. Empty checks nothing. A path that cannot be read or parsed at boot is fatal.

When the allowlist is unset, any certificate this CA has ever signed still reaches `POST /admin/login`, including one issued for a decommissioned operator or another component. The gateway logs a security warning naming this risk. That residual is what this row accepts.

**Why the residual is accepted:**

- **Upgrade compatibility:** Refusing to start without an allowlist would break every existing deployment on upgrade. The warning is the same shape as the key-retention warning on `cmd/vault`.
- **Single-purpose CA is still the floor:** `generate-admin-certs.sh` issues the CA exclusively for the admin gateway. An operator with one client certificate and no decommissioned identities can leave the allowlist empty.
- **The controls fail closed once set:** a name that is not on the list is a handshake error, and an unreadable, unparseable, foreign-signed or expired CRL refuses the handshake (and the boot, if the path is already bad then).
...
- **A sanctioned purge now says so in the log it purged.** Until 1.0.3 a completed sweep left only a process-log line, so a trail that stops abruptly read the same whether the sweeper ran on the configured horizon, ran on some other horizon, or somebody called `audit.cleanup_old_entries()` by hand. A sweep that deleted anything, or that failed partway, writes an `admin_action` entry naming the cutoff it actually applied, the number of rows, the configured horizon and the replica that held the election. It does not close this risk -- the caller who can purge can purge that row too, a horizon later -- but it removes the case where a purge and a silence are indistinguishable. Asserted by `TestRetention_PurgeIsRecordedInTheAuditLog`.
- **Still open at the SQL boundary.** A direct call to `audit.cleanup_old_entries()` leaves no entry, because the record is written by the Go sweeper rather than by the function. Closing that needs the attestation to move into the migration.
...
Both are closed. `Refresh` now opens every row before publishing any of it, and publishes only a row whose `private_key` decrypts under the master key with the kid as AAD and whose `public_key` column is the public half of what decrypted. Migration 017 freezes revoked rows against UPDATE and DELETE.

The grant has since moved, which this entry originally said it had not. Migration 037 took table-level UPDATE off both application roles and gave back `status`, `retired_at` and `expires_at` only, after moving `Import`'s upsert into `auth.import_signing_key`, a `SECURITY DEFINER` function on 015's pattern. The reason is this entry's first bullet read from the other side: a genuine re-import and a substitution of key material are the same row shape, and only the master key separates them, so that write had to be constrained by who may issue it rather than by what it says. `kid` additionally carries `signing_keys_kid_immutable`, because a privilege answers only for these two roles while the kid is the AEAD associated data of its own row and the identifier a tombstone retires, which is an invariant for every role. What remains is worth stating exactly.
...
- **What 037 closes is the invariant, not the availability.** A caller holding `vault_app` can still retire the active key through its lifecycle grant and then call `auth.import_signing_key` with a self-consistent `(kid, public_key)` pair of its own and ciphertext that does not open, which leaves an active row no pod can load. That is the same denial of service the bullet above already accepts. What no longer survives is the claim 026 and 035 made and could not keep -- that a rotated-out key stays retired, and that a tombstoned kid is never filled again -- because reaching a retired row's `private_key` or any row's `kid` by UPDATE is now a privilege error before any guard is consulted. The owner can still rewrite material on a reactivation, for the reason in the first bullet, and `TestSigningKeyRepublishAndReapReinsertAsVaultApp` pins that limit as a recorded fact rather than an assumption.
- **Guarded against regression.** `TestSigningKeyInjectionAsVaultApp` connects as the real `vault_app` role with the real grants and runs all three writes: the forged INSERT, the `public_key` swap by UPDATE, and the un-revoking UPDATE, checking each against both the published key set and a token forged under it. The swap is now refused twice over -- once by the privilege for `vault_app` and once by `Refresh`'s public-half comparison for every other role -- and the test asserts both, so removing either would be visible. `TestSigningKeyRevocationIsTerminalInTheDatabase` pins 017's trigger and, as importantly, that rotation, revocation and cleanup still work. `TestSigningKeyIdentityIsNotRawWritable` pins 037: renaming a retired, active or tombstone-adjacent kid, and reactivating a retired key under new material, each asserted against the trigger on the owner pool and against the privilege on a real `vault_app` connection, with the genuine re-import path and the SQL-versus-Go kid derivation as controls.
