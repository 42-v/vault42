...
| **CR-28** | Low | `GET /auth/verify-email` mutates state, and no route the vault serves validates `Sec-Fetch-*`. The token is single-use and consumed atomically, which bounds it to one verification. The emailed link is the SPA route, so a plain prefetcher changes nothing; a scanner that executes JavaScript does. | ASVS V3.5.3 |
| **CR-29** | Low | With `VAULT_SERVE_FRONTEND` on, the SPA and the API answer on one origin, so the same-origin policy separates neither. Off by default; the chart *offers* the SPA as a separate workload, behind `frontend.enabled` and an operator-supplied ingress host, rather than shipping it that way. | ASVS V3.5.4 |
| **CR-30** | Low | Admin email HTML is validated by a four-stage validator written for this codebase rather than by a named sanitisation library. `super_admin`-only, re-validated wherever a stored override is loaded, and an email body rather than a same-origin page are the compensating controls. Not `html/template` auto-escaping, which `internal/email/template_guard.go` is explicit contributes nothing against a hostile template *author*. | ASVS V1.3.1 |
...
| Added | Why it fits |
|---|---|
| **OWASP API Security Top 10:2023** | Its top two categories are object-level and function-level authorization, which is where an authentication service lives or dies. All ten Met. API7 was carried as an accepted risk alongside ASVS V1.3.6 until the four destinations an OIDC discovery document supplies -- the only ones vault42 takes from data -- were held to the issuer's own domain and judged again at dial time; see CR-17, retired. API9 is asserted rather than described: all 108 mounted routes must appear in `docs/api.md` and in the route inventory in `docs/spec.md`, checked on every build in both directions -- a route the source registers and no document lists, and a route a document lists that the source does not register. This row read "51" from 1.0.0 until 1.0.3, and 51 was never the number: the inventory held 105 routes on the day the sentence was written. A count in prose beside a gate that counts for itself is a claim with nothing behind it, so `TestRouteInventoryCountInProseIsCurrent` now checks this one. |
...
Eight accepted risks have closed, and in each case the test written to fail *on
closure* is what forced the row to move rather than a person remembering to.

CR-21 and CR-23 sat in the open table above until this release, under a body
that said in its own first sentence that they were closed in the code and still
open in the register. The register had already retired both. A row that argues
against its own table is the shape this section exists to prevent, so they are
here now and `TestComplianceDocDoesNotCarryARetiredRiskAsOpen` keeps them here.
...
| **CR-21** | `VaultClaims` carries `acr`, `amr` and `auth_time`, and `AALForMethods` has non-test callers on every user-authenticating issuance path: password login and MFA completion through `internal/service/auth.go`, and the OAuth callback through `internal/handler/oauth.go`. A resource server can require a specific authentication strength. | `TestASVS_V6_8_4_FederatedLoginAssumesTheMinimumAuthenticationStrength` and `TestASVS_V10_3_4_RecentnessIsVerifiedAgainstThePresentedAccessToken` |
| **CR-23** | `internal/email/smtp.go` replaced `net/smtp.SendMail` with an explicit conversation: STARTTLS when the server advertises it, `ErrSMTPNoSTARTTLS` when it does not, and `tls.VersionTLS12` rather than the Go default. The opt-out `VAULT_SMTP_ALLOW_PLAINTEXT` is refused outside the dev profile unless the SMTP host is loopback. **ASVS V12.3.1 did not become Met with it** -- the relay was never the only unencrypted outbound link, and the cache connection is the one that decides that requirement. | `TestASVS_V12_3_1_TheCacheLinkCanBeEncrypted`, which covers the requirement the closure did *not* satisfy |
