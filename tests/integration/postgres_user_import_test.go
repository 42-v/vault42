...
func TestPostgresUserImport(t *testing.T) {
	...
	legacy := randomID()
	u := &model.User{
		ID: randomID(), Email: "imported@beon3.test", DisplayName: "Imported",
		Locale: "en", Roles: []string{"moderator"}, Banned: true, BanReason: "spam",
		ImportedFrom: "beon3", LegacyID: legacy, CreatedAt: now, UpdatedAt: now,
	}
	...
	t.Run("CreateImported sets import_pending + no password", func(t *testing.T) {
		...
		if got.ImportedFrom != "beon3" || got.LegacyID != legacy {
