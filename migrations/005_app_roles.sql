...
-- admin-reserved deny-list (internal/seed). Consumers like the legacy platform need their own
...
-- the legacy platform application roles (deletable/manageable by super_admin).
INSERT INTO auth.app_roles (name, namespace, description, reserved) VALUES
    ('moderator',    'legacy', 'Forum moderator', FALSE),
    ('premium_user', 'legacy', 'Premium subscription user', FALSE),
    ('business',     'legacy', 'Business account', FALSE),
    ('creator',      'legacy', 'Content creator (dedicated forum subcategory)', FALSE)
ON CONFLICT (name) DO NOTHING;
