-- Add account-level lifecycle flags to auth.users for legacy-platform parity.
--
-- the legacy platform's Users table carries Active / Banned / BanReason / LastLogin and a
-- soft-delete. vault42 already had email_verified + locked_until; this adds the
-- remaining account-state columns so an imported the legacy platform account keeps its status,
